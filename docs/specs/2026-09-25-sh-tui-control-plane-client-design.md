...
- **§10's "cannot detect a harness without auth" was wrong.** A harness missing
  ...
  §8.2 and `sh-tui doctor` (§6.9) now diagnose it.
...
**Hard constraint, checkable rather than assumed:** `packages/tui/package.json` declares **no**
...
day-to-day invocation is plain `sh-tui`.
...
- `$XDG_STATE_HOME/sh-tui/transcripts/<sessionId>.jsonl` (fallback `~/.local/state/sh-tui/…`), one
...
- Deleting a session deletes its transcript. `sh-tui` never uploads a transcript anywhere.
...
- `$XDG_CONFIG_HOME/sh-tui/config.json` — `{ controlPlaneUrl, harnessUrl, theme, details, thinking,
reducedMotion, keybinds, presets }`.
- `$XDG_CONFIG_HOME/sh-tui/auth.json`, mode `0600` — `{ apiToken, subject, displayName, roles,
expiresAt }`. The only secret-shaped data stored, and a 1-hour capability, never a provider key.
- `$XDG_STATE_HOME/sh-tui/transcripts/` — §6.6.

### 6.8 First run — guided onboarding

When no `config.json` exists, `sh-tui` opens an onboarding overlay rather than an error:
...
Each step is skippable when already satisfied, so re-running onboarding (`sh-tui --setup`) on a
...
| 3   | logged in (`GET /v1/me`)                                                               | "not logged in — run `sh-tui` to log in"                                                                                                                   |
...
- **Live smoke** — env-gated (`TUI_LIVE_SMOKE=1`, following the repo's `*_LIVE_SMOKE` convention):
  `sh-tui doctor --json`, then `sh-tui run --new "…" --json` against a real control plane and harness,
