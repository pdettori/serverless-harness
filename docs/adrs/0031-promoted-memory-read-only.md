...
- Negative / accepted cost: a promoted run cannot accumulate knowledge across dispatches; every run starts from what the author taught it locally. Operational discoveries require a human in the loop to become durable, which is friction by design but is still friction. Memory files reaching the sandbox means they land on a shared pool's volume, so the user deny-list and the secret scan (`packages/config-bundle/src/secret-scan.ts`) are what stand between private context and a shared cluster before promotion: five structural rules (AWS/GitHub/OpenAI/Slack token and private-key-block shapes) block it outright, while two prose heuristics (bearer tokens, `key: value`-shaped assignments) only warn — measured against a real `~/.claude` the heuristics were false-positive-dominated, so only the structural tier is load-bearing. The overlay's `chmod -R a-w` pass (`harness/src/config-overlay.ts`) separately enforces this ADR's read-only guarantee at the filesystem level, so a bundle that clears the scan still cannot be mutated once it reaches the sandbox. Write-protection is not lifetime, though, and the two were conflated at first: the digest-keyed cache initially had no teardown at all, so promoted memory stayed on the shared volume after its leaf ended and a later leaf carrying no `configRef` could read it — breaking spec §2 goal 6 observably, since tools run in the sandbox ([#216](https://github.com/rossoctl/serverless-harness/issues/216)). The cache is therefore refcounted: each leaf registers a file under `/workspace/.sh-config/.refs/<digest>/` and the release drops the tree once the last one goes, all under the same per-pod `flock` the populate path uses. Concurrent fan-out still transfers a bundle once (spec §4.5); reuse across separate dispatches is given up deliberately, so a dispatch arriving after the pool went idle re-transfers. Concurrent cross-leaf visibility remains an accepted non-goal (P2 §9; Kata isolation is P3/#48) — what is now guaranteed is that promoted memory does not survive the leaf that brought it. And the read-on-demand path depends on the sandbox overlay succeeding, so a memory read failure surfaces as a tool miss rather than a configuration error.
