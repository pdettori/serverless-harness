...
describe("resolveTurnSandbox", () => {
  it("returns the injected sandbox verbatim, including its transport", async () => {
    const transport = { exec: async () => ({ stdout: Buffer.from(""), exitCode: 0, truncated: false }), close: async () => {} };
    const injected = { config: { pod: "sbx-laptop", namespace: "default", podCwd: "/workspace", headCwd: "/head" }, transport };
  ...
  it("ignores env resolution entirely when a sandbox is injected", async () => {
    ...
    const injected = { config: { pod: "sbx-leased", namespace: "default", podCwd: "/workspace", headCwd: "/head" } };
  ...
  it("falls back to env resolution when nothing is injected (the /turn path)", async () => {
    ...
    expect(got.config).toMatchObject({ pod: "sandbox-0", namespace: "default", podCwd: "/workspace", headCwd: "/head" });
