...
- `$XDG_STATE_HOME/sh-tui/transcripts/` — local session history, per subject and per control plane
  (mode 0600). A session resumed from another machine shows no history — it belongs to a
  different transcript store.

Falls back to `~/.config/sh-tui/` and `~/.local/state/sh-tui/` when the `XDG_*` variables are unset.
